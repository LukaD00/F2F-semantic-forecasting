...
class CityscapesHalfresGroundTruthDataset(Dataset):
	"""
	A Dataset object that groups together past feature tensors with corresponding ground truth
	...
	def __init__(self, num_past = 4, transform = None):
		"""
		...
		self.items = [] # array that stores groups of (past tensors, ground truth labels)
		...
		for ground_truth in truth_files:
			...
			for i in range(1, num_past+1):
				# past feature file name should be in the format "city_sequence_time_leftImg8bit.npy"
	...
	def __len__(self):
		"""
		Returns length of the dataset - more precisely, the amount of (past features, ground truth) items.
		"""
	...
	def __getitem__(self, idx):
		"""
		Returns the idx-th item in the dataset - a tensor representing a set of several past tensors, and labeled future ground truth.

		Args:
			idx (int): Index of the wanted item.

		Returns:
			((128 * num_past, 16, 32) torch.Tensor), ((1024, 512) np.array)
		"""
		item = self.items[idx]
		...
		return past_features, ground_truth